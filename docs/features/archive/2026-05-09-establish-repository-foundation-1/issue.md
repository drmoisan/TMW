...
23. Main-branch governance requirements are documented in `docs/branch-protection.md` and applied programmatically via `.github/scripts/apply-branch-protection.ps1`, which deletes the legacy branch-based protection rule for `main`, enables repository-level merge commits for pull requests (`allow_merge_commit=true`), and creates or updates a repository ruleset for `refs/heads/main` that requires the eight PR-pipeline contexts (`tier-classification`, `stage-1-format`, `stage-2-lint`, `stage-3-typecheck`, `stage-4-architecture`, `stage-5-test`, `stage-6-contract`, `stage-7-integration`) with strict status checks. The modern ruleset does not require an approving review and does not require linear history. Verification: `gh api -X GET repos/drmoisan/TMW` reports `allow_merge_commit=true`, `gh api -X GET repos/drmoisan/TMW/branches/main/protection` returns `404`, and `gh api -X GET repos/drmoisan/TMW/rulesets` returns the active `Main branch PR governance` ruleset.
