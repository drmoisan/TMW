# Code Review: iFile Message-Filing Dialog (#43) — Cycle 2 Reaudit (POL-1 re-evaluation)
...
**Supersedes:** `code-review.2026-06-04T21-02.md` (left in place for the audit trail). Re-issued to
reflect the POL-1 re-evaluation. No code finding changed; the single externally-blocking item cited in
T21-02 (the C# coverage artifact) is resolved by ground-truth evidence and is no longer blocking.
...
forbidden set for pure modules), both manifests, `package.json` (MSAL dependency). The pre-existing
working-tree `Program.cs`/`csproj` edits (OBO/Graph DI wiring + whitespace) are present in the diff but
are not cycle-2 deliverables. New tests cover the NAA adapter, the bootstrap seam, the host shell, and
the URL guard.

**Top 3 risks:**
1. The NAA-unsupported fallback names the Office-dialog interactive flow, which is an explicit out-of-scope follow-up (not yet built) — acceptable and deterministic, but the on-device unsupported path remains unverified until HI-2.
2. Final tenancy (`common` vs single-tenant authority) is governed by the human-gated HI-3 Entra config, not by this code.
3. The CI C# coverage artifact is a single-project file, not a merged repo-wide figure (policy-audit POL-1 re-evaluation); this is a measurement/CI-methodology matter, not a code defect, and is recorded as a separate out-of-scope follow-up.

**PR readiness recommendation:** **Go** — the code is sound and there is no remaining blocking item.
The C# coverage concern cited in the T21-02 review was an evidence-interpretation gap (single-project
artifact read as repo-wide), now resolved by ground-truth evidence; it is not a code change.
...
| Info | `.github/actions/dotnet-test/action.yml` (out of iFile scope) | "Emit canonical coverage artifact" step | CI selects one project's `coverage.cobertura.xml` by LastWriteTime instead of merging; the artifact is not repo-wide. | Record as a separate follow-up (ReportGenerator merge); touches `.github/actions/**` and triggers `modified-workflow-needs-green-run`. | Root cause of the T21-02 repo-wide misread; pre-existing, not a cycle-2 change. | `action.yml` lines 19-32; evidence T21-23. |
...
- The CORS edit is a formatting reflow with no behavior change; `csproj` change is whitespace only.
  These are pre-existing working-tree edits, not cycle-2 deliverables.

#### Error handling and logging

- No new error paths introduced; the wiring relies on the framework's existing validation.
- Coverage of the changed lines: the changed `Program` lines are 87% covered with no changed-line
  regression (full-suite ground truth, evidence T21-23). The previously-cited "stale/low repo-wide C#
  coverage" was a single-project-artifact misread (policy-audit POL-1 re-evaluation), not a code or
  coverage defect attributable to this cycle. Cross-reference only, not a code-review FAIL.
...
- `evidence/qa-gates/2026-06-04T21-23/csharp-coverage-methodology.md` — full backend suite ground truth: 145 tests pass; build `--warnaserror` clean.
...
No external research was required for this review. Verdicts derive from re-running the repository
toolchain, inspecting the diff and feature-folder artifacts, and verifying the orchestrator-supplied
ground-truth coverage evidence (T21-23) against `.github/actions/dotnet-test/action.yml` and
`git diff`.
...
The item cited in the T21-02 review as the single blocking concern (the C# coverage artifact) is
resolved by ground-truth evidence: the CI artifact is a single-project file, not a repo-wide figure,
and cycle 2 changed no C# production lines (the pre-existing `Program.cs` edit's changed lines are 87%
covered with no regression). That is a CI-measurement matter recorded as a separate out-of-scope
follow-up, not a code defect. Recommendation: **Go.**
